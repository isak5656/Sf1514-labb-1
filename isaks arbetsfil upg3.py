...
def uppgift_C ():
    ...
    def f(x):
        ...
        return funktion
   # -8 då 2014-2022=8 och det är 9 mätpunkter
    N = (len(t_data)) -1
...
def uppgift_D ():
    ...
    def f(x):
        ...
        return funktion
    #väljer h
    intervall = [1, 2, 4, 8, 16, 32]
    lagring = np.ones(len(intervall))
    for i in range(len(intervall)):
        N = intervall[i]
        #gränsen första sita t
        gränser = [t_data[0], (t_data[-1])]
        ...
        Th = trapets(gränser, N, f)
        lagring[i] = Th
        print(f"värdet från uppgift D integral är,{Th}, då steglängden är {intervall[i]}")

    "räkna u t flet e"
    eh1 = lagring[0] - lagring[2]
    eh2 = lagring[2] - lagring[3]
    eh3 = lagring[3] - lagring[4]
    print(f" felen i D\n här är felet för h = 5, 10 {eh1} \n här är felet för h = 10, 20 {eh2}\n här är felet för h = 20, 40 {eh3}")
    return lagring, intervall
